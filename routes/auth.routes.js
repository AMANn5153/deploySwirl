...
router.get('/google/callback', (req, res, next) => {
  passport.authenticate('google', (err, user, info) => {
    if (err) {
      ...
      return res.redirect(`https://swirl-1ms3.onrender.com/Login?error=${encodeURIComponent(err.message)}`);
    }

    req.logIn(user, { session: false }, (err) => {
      if (err) {
        console.log("Error occurred during login:", err);
        return res.redirect(`https://swirl-1ms3.onrender.com/Login?error=${encodeURIComponent('An error occurred during login')}`);
      }
      ...
      res.redirect(`https://swirl-1ms3.onrender.com/Login?user=${userData}`);
    });
  })(req, res, next);
});
